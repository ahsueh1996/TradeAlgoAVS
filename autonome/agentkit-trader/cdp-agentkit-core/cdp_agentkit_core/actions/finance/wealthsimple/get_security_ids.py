...
This tool will help get the Wealthsimple security ids for some symbols. You can use this tool to resolve the security id for a given symbol.
You will use this tool when infering which security the user is talking about.
For example, if the user says "fetch the details for AAPL", you will use this tool to resolve the security id for AAPL FIRST.
...
def get_security_ids(client: wspy.Client) -> str:
    ...
    """
    ids = client.get_security_ids()
    print(f"GOT WS SecIDS: {ids}")
    return ids


class GetSecurityIDsAction(WealthsimpleAction):
